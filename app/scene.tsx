...
interface Props {atlas:Atlas;state:SceneState;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(key:MessageKey)=>void}
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 const {t}=useI18n();
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect),translate=useRef(t);
 latest.current=state;select.current=onSelect;translate.current=t;
 useEffect(()=>{
  const el=host.current!;let disposed=false,frame=0,dirty=true,ready=false,lastView='',lastReset=-1,lastIsolate='',layoutKey='',amount=0;
  let lastState:SceneState|null=null;
  const abort=new AbortController();
  let renderer:T.WebGLRenderer;
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError('scene.webgl');return;}
  ...
  renderer.domElement.setAttribute('aria-label',translate.current('scene.canvasLabel'));
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('scene.assembleFailed');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error&&e.message==='scene.assembleFailed'?'scene.assembleFailed':'scene.loadFailed');}})();
  ...
  const animate=()=>{
   ...
   controls.zoomToCursor=s.zoomToCursor;controls.enableRotate=amount<.8;controls.mouseButtons.LEFT=amount<.8?T.MOUSE.ROTATE:T.MOUSE.PAN;controls.touches.ONE=amount<.8?T.TOUCH.ROTATE:T.TOUCH.PAN;ground.visible=platform.visible=ring.visible=innerRing.visible=amount<.5&&!s.isolate;markers.visible=amount>.75;controls.autoRotate=s.rotate&&!s.isolate&&amount<.4;controls.autoRotateSpeed=.65;controls.update();if(controls.autoRotate)dirty=true;
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError('scene.contextLost');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
 ...
 // Met a jour le libelle d'accessibilite du canevas sans reconstruire la scene 3D.
 useEffect(()=>{host.current?.querySelector('canvas')?.setAttribute('aria-label',t('scene.canvasLabel'));},[t]);
